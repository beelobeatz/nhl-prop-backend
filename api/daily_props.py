...
def get_daily_props():
    try:
        # Keys from environment (set in Vercel dashboard)
        BALLDONTLIE_KEY = os.environ["BALLDONTLIE_KEY"]
        GROK_API_KEY = os.environ["GROK_API_KEY"]

        today = datetime.utcnow().strftime('%Y-%m-%d')  # Adjust if needed for ET
